...
export const AddInstallation = () => {
    ...
    const handleSubmit = async (event) => {
        ...
        try {
            const response = await axios.post("http://api.sigusminera.online/api/installations_numbers/add", newInstall);
            toast.dismiss();
    ...
    useEffect(() => {
        const fetchTechnical = async () => {
            const res = await axios.get('http://api.sigusminera.online/api/company/');
