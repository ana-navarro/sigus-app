...
export const ShowCredit = () => {
    ...
    const getInstallationNumber = async (id) => {
        try {
            const res = await axios.get(`http://sigus-server.vercel.app/api/installations_numbers/${id}`)
    ...
    const getAddressData = async (id) => {
        try {
            const res = await axios.get(`http://sigus-server.vercel.app/api/company/${id}/address`);
    ...
    const sendLink = async (e) => {
        e.preventDefault();
        const res = await axios.post(`http://sigus-server.vercel.app/api/credit/${id.idCredit}/send-demo`, email);
        if (res.status === 201) {
