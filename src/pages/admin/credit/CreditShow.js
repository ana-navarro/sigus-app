...
export const ShowCredit = () => {
    ...
    const getAddressData = async (id) => {
        try {
            const res = await axios.get(`http://181.215.134.184/api/company/${id}/address`);
    ...
    const sendLink = async (e) => {
        e.preventDefault();
        const res = await axios.post(`http://181.215.134.184/api/credit/${id.idCredit}/send-demo`, email);
        if (res.status === 201) {
