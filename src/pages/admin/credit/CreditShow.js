...
export const ShowCredit = () => {
    ...
    const getInstallationNumber = async (id) => {
        try {
            const res = await axios.get(`http://localhost:5000/api/installations_numbers/${id}`)
    ...
    const getAddressData = async (id) => {
        try {
            const res = await axios.get(`http://localhost:5000/api/company/${id}/address`);
    ...
    const getData = async () => {
        try {
            const res = await axios.get(`http://localhost:5000/api/credit/${id.idCredit}`);
            setCredit(res.data);
    ...
    const sendLink = async (e) => {
        e.preventDefault();
        const res = await axios.post(`http://localhost:5000/api/credit/${id.idCredit}/send-demo`, email);
        if (res.status === 201) {
