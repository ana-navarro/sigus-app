...
export const ShowClient = () => {
    ...
    const getData = async () => {
        try {
            const res = await axios.get(`http://sigus-server.vercel.app/api/client/${id.idClient}`);
            setClient(res.data);
    ...
    useEffect(() => {
        const getCompanyName = async () => {
            try {
                const res = await axios.get(`http://sigus-server.vercel.app/api/company/${company}`)
